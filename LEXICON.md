...
export const DIRECTNESS: LexEntry[] = [
  ...
  { pattern: /^\s*(?:hey\s+@?\w+[,:]?\s*)?(send|share|forward|upload|post|attach)\b(?!ing)/i, weight: 10.0, category: "directness", subcategory: "mood_derivable", note: "Bare imperative with a transfer verb in clause-initial position — grammatical mood alone encodes the directive, the most direct realization available in English." },
  { pattern: /^\s*(?:hey\s+@?\w+[,:]?\s*)?(review|check|confirm|approve|sign|verify|look at)\b(?!ing)/i, weight: 10.0, category: "directness", subcategory: "mood_derivable", note: "Imperative with an evaluative verb; identical mood-derivable structure, listed separately so evidence subcategories stay interpretable in the UI." },
  { pattern: /^\s*(?:hey\s+@?\w+[,:]?\s*)?(fix|update|add|remove|delete|change|rename|merge|push|deploy|revert)\b(?!ing)/i, weight: 10.0, category: "directness", subcategory: "mood_derivable", note: "Imperative with an action-on-artifact verb — the dominant imperative frame in engineering channels." },
  ...
  { pattern: /\bjust wanted to ask\b/i, weight: 8.0, category: "directness", subcategory: "hedged_performative", note: "Chat-register hedged performative; *just* and the past tense are separately scored as downgraders since neither is contained in the minimal performative span." },
  ...
  { pattern: /\bI (would |'?d )?(really )?like\b(?! to ask)/i, weight: 6.0, category: "directness", subcategory: "want", note: "States the speaker's desire and leaves the inference to the hearer; the conditional is absorbed under §0.2." },
